...
### Save in file
```{r}
write.xlsx(data, "CoV2_UN_48_filtered.xlsx", row.names = FALSE)
```
...
```{r}
write.xlsx(enrichr_sig, "Enrichr_sig.xlsx", sheetName = "Enrichr_sig", col.names= TRUE, row.names = FALSE)
```
...
```
#### Building top20 bar plot

```{r}
enrichr_top$Term <- factor(enrichr_top$Term, levels = enrichr_top$Term ) #To make it the right order for ggplot

pdf("Comparison_pathways_enrichr_top20_bar.pdf", width = 19, height = 20)
p<-ggplot(data = enrichr_top, aes(x = Term, y = Overlap)) +
  geom_bar(stat="identity", aes(fill = Adjusted.P.value)) + 
  scale_fill_gradientn(colors = c("#668466", "#CC0000")) + 
